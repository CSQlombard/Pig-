...
count_by_subject = FOREACH subjects GENERATE $0, COUNT($1) PARALLEL 50;

-- Group by counter
counter_group = GROUP count_by_subject BY $1 PARALLEL 50;

final_count = FOREACH counter_group GENERATE $0, COUNT($1) PARALLEL 50;
...
--store final_count_reduce into 's3://aws-logs-277243302517-us-west-2/elasticmapreduce/problem_2b_HIST_a' using PigStorage(','); -- s3 bucket direction.
--store final_count_reduce_b into 's3://aws-logs-277243302517-us-west-2/elasticmapreduce/problem_2b_HIST_b' using PigStorage(','); -- s3 bucket direction.
--store final_count_reduce_b into 'hdfs://localhost:54310/user/hduser/problem_2b_HIST_b' using PigStorage('\t');
